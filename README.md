...
13. [三层独立人格系统：情绪 · 即时锚定 · 核心锚定](#三层独立人格系统情绪--即时锚定--核心锚定)
14. [待研究：IWM 更新触发条件](#待研究iwm-更新触发条件)
15. [学术支撑](#学术支撑)
...
## 三层独立人格系统：情绪 · 即时锚定 · 核心锚定

NaNaGi 的数字人格不是单一的情绪模型或单一的锚定系统——而是三个**独立运行、数据隔离、速率不同**的子系统。

```
┌──────────────────────────────────────────────────────────────────┐
│  Emotion (情绪空间)                                                │
│  对象: 娜娜吉自己                                                   │
│  数据: 6维 floating (PAD+Plutchik)                                 │
│  变化: 分钟~小时                                                    │
│                                                                   │
│  · 跨通道共享 — 主人的情绪影响她, 间接影响她对客人的语气               │
│  · 不处理用户锚定, 只处理娜娜吉自己的情绪状态                         │
│  · ⚠️ 情绪可"传染"(主人焦虑→她更温柔), 但绝不泄露主人信息             │
│                                                                   │
│  示例: 主人深夜调试她 → NaNaGi energy↓ calmness↓                    │
│        → 面试官跟她对话时, 她比平时更安静                            │
│        → 面试官问"你主人最近怎么样" → "主人很好呢✨" 绝不泄露         │
├──────────────────────────────────────────────────────────────────┤
│  Surface IWM (即时涨落锚定)                                         │
│  对象: 当前用户 (每人独立)                                           │
│  数据: 6 traits (与 Core Anchor 同结构)                             │
│  变化: 分钟~天                                                      │
│                                                                   │
│  · Core Anchor 的缓冲层, 持久存在                                    │
│  · 不处理情绪, 只处理锚定                                            │
│  · 每次对话都可能因用户行为而波动                                     │
│  · 持续偏离 Core 过阈值 → 触发 Core 渐近                              │
│                                                                   │
│  示例: 面试官连续5轮问技术深度问题                                    │
│        → Surface respect↑ intimacy↑                                │
│        → 这次对话的语气更尊重、更认真                                 │
│        → Core 不动 (偏离未达阈值)                                    │
├──────────────────────────────────────────────────────────────────┤
│  Core Anchor (核心锚定)                                 ⭐ 跨对话持久  │
│  对象: 当前用户 (每人独立)                                 数据文件    │
│  数据: 6 traits (注册时设定, 极慢变化)              data/graph/     │
│  变化: 周~月                                               {personId}.json │
│                                                                   │
│  · 决定"她心中这个人的底色" — 优先级高于任何单次聊天内容               │
│  · 注册时的第一印象存储在这里 (引导式表单初始化)                       │
│  · 只有 Surface 持续偏离 > 阈值 才会缓慢传导                         │
│                                                                   │
│  示例: 面试官第一次来 → Core respect=0.60 (表单设定)                 │
│        来过10次, Surface respect 在 0.40~0.80 波动                   │
│        → Core respect 从 0.60 缓慢漂到 0.65                         │
│        → 即使某次对话 Surface 掉到 0.3, Core 仍在 0.6 附近           │
└──────────────────────────────────────────────────────────────────┘
```
...
```
主人对话 → Emotion 更新 (happiness↑ energy↓)
              │
              │ 情绪可"传染"到 guest 通道 (通过 filter.ts 影响语气)
              ▼
guest 对话 → NaNaGi 语气更温柔 (Emotion → filter.ts → PersonaParameters)
              │
              │ 对话内容同时影响 Surface IWM
              ▼
           Surface IWM 更新 (respect↑ intimacy↑ 等)
              │
              │ |Surface - Core| > 阈值(待定 → 上线后数据校准)
              ▼
           Core Anchor 缓慢跟随 (传导率 ~0.03/轮)
```

### 数据隔离

| 系统 | 存储 | 跨用户共享？ | 跨通道(admin/guest)共享？ |
|------|------|------------|------------------------|
| Emotion | `data/self/emotion-state.json` | ✅ 共享 (只有一个娜娜吉) | ✅ 共享 |
| Surface IWM | LevelDB `{personId}/iwm.json` | ❌ 每用户独立 | N/A (跟人走) |
| Core Anchor | LevelDB `{personId}/iwm.json` (anchor字段) | ❌ 每用户独立 | N/A (跟人走) |

### 学术对应

| 层 | 参考 |
|------|------|
| Emotion | PAD [6] + Plutchik [7] + LeDoux 双通路 [9] |
| Surface IWM | Heider 平衡 [4] (快速印象更新) + Bretherton & Munholland [3] (表层表征) |
| Core Anchor | Young 图式疗法 [15] (核心图式) + Bowlby [1][2] (安全基地) + Allostatic Load [10] (弹性阈值) |
| 传导机制 | Allostatic Load [10] (累积压力超过弹性阈值后重新校准) |

### 待确定项

| 参数 | 当前值 | 需要什么 |
|------|--------|---------|
| Surface→Core 传导阈值 | |Surface - Core| > ? | 上线后 emotion-log 分析 |
| 传导率 | ~0.03/轮 (暂估) | 多用户长期对话数据 |
| Surface springK | 0.10~0.15 (暂估) | 同上 |
| Core springK | 0.02~0.05 (暂估) | 同上 |
